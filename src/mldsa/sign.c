...
* Returns 0 (success)
**************************************************/
/* Xyntetik: keys derive from a caller-supplied 32-byte seed (the signkey
 * file stores the seed, as the Ed25519 path does), so the OS random source
 * is never consulted here and a key file reproduces its public key. This is
 * FIPS 204 ML-DSA.KeyGen_internal with xi = seed. */
int crypto_sign_keypair_seed(uint8_t *pk, uint8_t *sk, const uint8_t *seed) {
...
#if 0 /* Xyntetik: deterministic signing only; receipts must reproduce */
#else
