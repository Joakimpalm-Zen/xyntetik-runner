...
| `--transcript FILE` | Record a one-shot `-p` run as `xyntetik.runner.transcript.v1`: model, adapter and binary hashes; execution profile; the exact 64-bit seed and sampling config; prompt/output text and token ids; and a chain hash over the serialized record body. |
