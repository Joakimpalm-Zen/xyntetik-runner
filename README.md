...
> **Pre-1.0 (`0.4.3`).** APIs, model coverage and certification envelopes may
...
| `--draft-required` | Fail the run instead of decoding plain when `--draft` is refused. The drop is deliberate and stays the default, but in one-shot mode it is announced only on stderr beside a zero exit, so a harness that collects stdout and checks the return code measures the unaccelerated baseline and records it as speculative decoding. This flag closes that hole for benchmarking; it needs `--draft`, and it is refused in serve mode rather than accepted with no effect, because `GET /v1/capabilities` already reports whether the draft is `active` there. |
...
| `RUNNER_KV_RING` | unset | Give sliding-window layers only the KV rows they can read, indexed modulo that count, instead of a full `n_ctx` rows each. A local layer never attends past its window, so the rest of its cache is written once and never read: on gemma-3-4b at `-c 32768` this takes the cache from 4563 MB to 800 MB, within 1% of the theoretical floor. Output is unchanged - the ring holds exactly the rows the flat allocation would have been read from, gated as bit-identical `--score` logprobs against the default path. **It is opt-in because it costs something, and it is CPU-only:** the prefix cache and partial rewind address KV as flat absolute rows, so both are refused while a ring is active and a server loses shared-prompt reuse; and the device attention kernels address the cache by absolute position too, so the ring is refused with a message when a GPU is in play rather than returning wrong numbers (measured: a ring on an RTX 3070 partial split produces `nan` for every scored position). Worth it when context length is the binding constraint on a CPU run, not otherwise. Dense and full-attention-only models ignore it. |
